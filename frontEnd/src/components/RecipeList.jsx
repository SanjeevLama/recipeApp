...
const placeholderImages = ["/recipe_1.png", "/recipe_2.png", "/recipe_3.png"];
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {paginationData.results.map((recipe, index) => {
          // Get the index of the recipe in the array
      ...
      <div className="flex justify-center items-center space-x-4 mt-8">
        <button
          ...
          className="py-2 px-4 bg-blue-500 text-white rounded disabled:bg-gray-300 cursor-pointer"
        >
          Previous
        </button>
        <span className="text-gray-700">
          ...
          {paginationData.previous
            ? getPageNumber(paginationData.previous) + 1
            : 1}{" "}
          of{" "}
          {paginationData.count > 0
            ? Math.ceil(paginationData.count / paginationData.results.length)
            : 1}
        </span>
        <button
